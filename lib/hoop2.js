...
let typeId = 1n
const roleMask = 0b1111111111n // ten bits
const typeMask = ~roleMask

const token = (...source) => (role, ...info) =>
  [String.raw (...source) .replace (/\s+/g, ''), (typeId++ << 10n) | role, ...info]

const tokenType = () => ['', typeId++ << 10n] // HACK to produce typeIds, for now
const newTag = () => typeId++ << 10n

// ### Tokens Roles
// Using bitflags for the token roles. 

const FlagsOnto = (map = {}, start = 0n) =>
  new Proxy ({}, { get:($,k) => map [k] || (map[k] = 1n << start++) })

const Roles = { }
const { START, END, SKIP, CONST, LEAF, ASSOC, INFIX, PREFIX, POSTFIX, GROUP } = FlagsOnto (Roles)

const _token = role => (...source) =>
  [String.raw (...source) .replace (/\s+/g, ''), role]

const [start, atom, konst, prefix, infix, assoc, postfix, end] = 
  [START, LEAF, CONST, PREFIX, INFIX, ASSOC, POSTFIX, END] .map (_token)

const opInfo = op => {
  const roles = []
  for (let k in Roles) if (op & Roles [k]) roles.push (k)
  return roles.join(' | ')
}
...
function compileRule (ruleName, rule, grammar) {
  ...
  for (let i=0, l=sig.length; i<l; i++) for (const k in sig[i]) {
    ...
    if (typeof def[0] === 'bigint') { // wrapfix operator (WIP -- add end too)
      ...
      types[k] = t |= role
      info = [rx, t]
      // log ('compile.... ', t, opInfo(t), info, t &~ (LEAF | CONST))
      if (role &~ (LEAF | CONST)) info[2] = i // set precedence
    }

    if (info[1] & (PREFIX | LEAF | CONST)) befores.push (info)
...
function Parser (lexers, startToken, endToken, apply = (...args) => args) {
  ...
  function parse (_input) {
    ...
    else if (role & (LEAF | CONST)) { // TODO Err if state is After
...
module.exports = { 
  compile, opInfo,
  Parser, Roles, token, tokenType, roleMask, typeMask,
  start, atom, konst, prefix, infix, assoc, postfix, end 
}