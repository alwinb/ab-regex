...
let typeId = 1n
const roleMask = 0b111111111n // nine bits
const typeMask = ~roleMask

const token = (...source) => (role, ...info) =>
  [String.raw (...source) .replace (/\s+/g, ''), (typeId++ << 9n) | role, ...info]

const tokenType = () => ['', typeId++ << 9n] // HACK to produce typeIds, for now
const newTag = () => typeId++ << 9n
